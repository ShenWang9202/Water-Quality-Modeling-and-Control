...
IndirectedControlledJunctionID = {'J211','J213','J215','J219','J225','229','J231','J239',...
    'J239','J249','J243','J255','J251',...
    'J50','J253'};%NodeID';
InterestedID = unique(IndirectedControlledJunctionID);
IDcell = NodeID;
[~,n] = size(InterestedID);
IndirectedControlledJunctionIndices = [];
for i = 1:n
    % find index according to ID.
    IndirectedControlledJunctionIndices = [IndirectedControlledJunctionIndices findIndexByID(InterestedID{i},IDcell)];
end
...
IndirectedControlledPipeID = {'P245','P247','P249','P251','P257','P261',...
    'P263','P275','P281','P289',...
    'P291','P293','P295','P283','P277'
    };
InterestedID = unique(IndirectedControlledPipeID);
IDcell = LinkID;
[~,n] = size(InterestedID);
IndirectedControlledPipeIndices = [];
for i = 1:n
    IndirectedControlledPipeIndices = [IndirectedControlledPipeIndices findIndexByID(InterestedID{i},IDcell)];
end
...
y = X_node_control_result(:,IndirectedControlledJunctionIndices);
meanData = mean(y,2);
[m,~] = size(y);
...
%% no control
y = X_node_control_result_no_control(:,IndirectedControlledJunctionIndices);
meanData = mean(y,2);
[m,~] = size(y);
...
legendJunctionID = ['Range Zone';'Average Value (booster)';temp;'Range Zone (uncontrolled)';'Average Value (uncontrolled)'];
...
y = X_link_control_result(:,IndirectedControlledPipeIndices);
meanData = mean(y,2);
[m,~] = size(y);
...
y = X_link_control_result_no_control(:,IndirectedControlledPipeIndices);
meanData = mean(y,2);
[m,~] = size(y);
...
print(fig,['Controlled_Net3',InterestedID{1}],'-depsc2','-r300');
print(fig,['Controlled_Net3',InterestedID{1}],'-dpng','-r300');